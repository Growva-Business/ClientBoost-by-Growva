          ...
          <div className="p-8">
            <div className="flex items-end justify-between h-48 gap-4">
              {analytics.revenue_by_month.length > 0 ? analytics.revenue_by_month.map((item: any, index: number) => {
                const maxRevenue = Math.max(...analytics.revenue_by_month.map((r: any) => r.revenue));
                const height = (item.revenue / maxRevenue) * 100;
                return (
                  <div key={index} className="flex-1 flex flex-col items-center gap-2">
                    <div className="flex-1 w-full flex items-end">
                      <div 
                        className="w-full rounded-t-lg bg-gradient-to-t from-rose-600 to-rose-400 transition-all hover:brightness-110"
                        style={{ height: `${height}%` }}
                      />
                    </div>
          ...
          <div className="divide-y divide-gray-50">
            {analytics.top_services.length > 0 ? analytics.top_services.map((service: any, index: number) => (
              <div key={index} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50/50 transition-colors">
                ...
                <p className="font-black text-sm text-gray-900">{salonProfile?.currency || '$'} {service.revenue.toLocaleString()}</p>
              </div>
            )) : (
          ...
          <div className="divide-y divide-gray-50">
            {analytics.top_staff.length > 0 ? analytics.top_staff.map((staff: any, index: number) => (
              <div key={index} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50/50 transition-colors">
                ...
                <p className="font-black text-sm text-gray-900">{salonProfile?.currency || '$'} {staff.revenue.toLocaleString()}</p>
              </div>
            )) : (
